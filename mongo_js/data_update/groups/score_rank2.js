// 西甲、意甲球队各个最新赛事比赛的积分统计统计更新 2014-03-05
...
cgs.forEach(function(item) {
  ...
  if(type2.indexOf(pid)>-1){

    ids_type2.push(item['_id']);

  }

})

league_info = db.current_groups.find({"_id":{"$in":ids_type2},"league_type":{"$in":[0,1]}},{"_id":1,"league_name":1});
...
// 积分统计函数开始
                ...
                function score_count(xx){
                    ...
                    eval('var field2={'+ xx +':team_id,"full_home_score":{"$ne":null},"full_away_score":{"$ne":null},"$where":"this.full_home_score>this.full_away_score","_id":{"$in": m_ids}}');

                    eval('var field3={'+ xx +':team_id,"full_home_score":{"$ne":null},"full_away_score":{"$ne":null},"$where":"this.full_home_score==this.full_away_score","_id":{"$in": m_ids}}');

                    eval('var field4={'+ xx +':team_id,"full_home_score":{"$ne":null},"full_away_score":{"$ne":null},"$where":"this.full_home_score<this.full_away_score","_id":{"$in": m_ids}}');
                    ...
                    eval('var field5={'+ xx +':team_id,"full_home_score":{"$ne":null},"full_away_score":{"$ne":null},"_id":{"$in": m_ids}}');

                    eval('var field6={"full_away_score":1,"full_home_score":1,"_id":0}');
