  ...
  for (var i = 0, len = s.length; i < len; i++) {
    ...
    for (var j = 0, jlen = gs.length; j < jlen; j++) {
      ...
      for (var id in mn_ids) {
              ...
              home_team_w_num = db.match.find({"_id": {"$in":m_ids},"home_team_id":team_id,"half_home_score":{"$ne":null},"half_away_score":{"$ne":null},"$where":"this.half_home_score>this.half_away_score"},{_id:1}).count();

              home_team_d_num = db.match.find({"_id": {"$in":m_ids},"home_team_id":team_id,"half_home_score":{"$ne":null},"half_away_score":{"$ne":null},"$where":"this.half_home_score==this.half_away_score"},{_id:1}).count();

              home_team_l_num = db.match.find({"_id": {"$in":m_ids},"home_team_id":team_id,"half_home_score":{"$ne":null},"half_away_score":{"$ne":null},"$where":"this.half_home_score<this.half_away_score"},{_id:1}).count();

              home_match_times=home_team_w_num+home_team_d_num+home_team_l_num;

              m = db.match.find({"_id": {"$in":m_ids},"half_home_score":{"$ne":null},"half_away_score":{"$ne":null},"home_team_id": team_id}, {"_id": 0,"half_home_score":1,"half_away_score":1});
              ...
              away_team_w_num = db.match.find({"_id": {"$in":m_ids},"away_team_id":team_id,"half_home_score":{"$ne":null},"half_away_score":{"$ne":null},"$where":"this.half_home_score<this.half_away_score"},{_id:1}).count();

              away_team_d_num = db.match.find({"_id": {"$in":m_ids},"away_team_id":team_id,"half_home_score":{"$ne":null},"half_away_score":{"$ne":null},"$where":"this.half_home_score==this.half_away_score"},{_id:1}).count();

              away_team_l_num = db.match.find({"_id": {"$in":m_ids},"away_team_id":team_id,"half_home_score":{"$ne":null},"half_away_score":{"$ne":null},"$where":"this.half_home_score>this.half_away_score"},{_id:1}).count();

              away_match_times=away_team_w_num+away_team_d_num+away_team_l_num;

              n = db.match.find({"_id": {"$in":m_ids},"half_home_score":{"$ne":null},"half_away_score":{"$ne":null},"away_team_id": team_id}, {"_id": 0,"half_home_score":1,"half_away_score":1});
              ...
              a=item.half_away_score;

              A2.push(a);

              b=item.half_home_score;
      ...
      function box3(x){
           ...
           function teamscore(x,y){
               ...
               matches = db.match.find({"_id":{"$in": m_ids},"home_team_id":home_team_id,"away_team_id":away_team_id},{"_id":0,"half_home_score":1,"half_away_score":1,"home_team_id":1,"away_team_id":1});
